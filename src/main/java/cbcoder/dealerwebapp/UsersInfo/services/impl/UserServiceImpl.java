 ...
 * The user will only can have the authority to update their first name, last name and password.
 * In the controller class we will determine the role of the user and based on the role we will provide the access to the methods.
...
public class UserServiceImpl implements UserService {
	...
	/**
	 * This method only have the accesses to the admin to update any user information. The user id and the userDto will be passed as a parameter.
	 ...
	 * If the email is not matching with the user email then it will throw an exception. Because the email should be matching with the user email to avoid update the wrong user information.
	 * If the user is not found with the given id then it will throw an exception.
	 * In this method the admin have the authority to enable or disable the user.
	 * And the last the admin can update a user role to another role, in case the user need change the department.
	 *
	 ...
	 * The user information will be updated only if the email is matching with the user email.
	 * If the email is not matching with the user email then it will throw an exception.
	 * If the user is not found with the given id then it will throw an exception.
	...
	public String deleteUser(Long userId) {
		Optional<User> userOptional = userRepository.findById(userId);
		if (userOptional.isPresent()) {
			if (userOptional.get().getRoles().stream().anyMatch(role -> role.getRoleName().equals(RoleEnum.ROLE_ADMIN))) {
				throw new OperationNotPermittedException("User with id " + userId + " is a ADMIN and cannot be deleted!");
			}
	 ...
	 * If no users found in the database then it will throw an exception.
