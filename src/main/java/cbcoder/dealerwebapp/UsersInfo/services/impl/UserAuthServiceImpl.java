...
public class UserAuthServiceImpl implements UserAuthService {
     ...
     * Roles will be set at the moment of user register. The role choices are SALES, WORKSHOP, VALETER.
     * The SUPERADMIN is set at database level. Then the SUPERADMIN can set the ADMIN role to other users.
     *
     ...
     * The method also has checks for user enabled status. If the user is not enabled, an exception is thrown.
     * At the moment of user register, the user is enabled by default. The user can be disabled or enabled back by the admin.
     *
